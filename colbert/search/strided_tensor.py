...
class StridedTensor(StridedTensorCore):
    ...
    def lookup(self, pids, output='packed'):
        ...
        if self.use_gpu:
            ...
            for i in range(1):
                num_chunks = 10
                chunk_size = tensor.shape[1] // num_chunks

                result_tensors = []
                for i in range(num_chunks):
                    start_idx = i * chunk_size
                    end_idx = start_idx + chunk_size if i < num_chunks - 1 else tensor.shape[1]
                    # Apply mask to each chunk
                    chunk_tensor = tensor[:, start_idx:end_idx]
                    chunk_mask = mask[:, start_idx:end_idx]
                    selected_elements = chunk_tensor[chunk_mask]
                    result_tensors.append(selected_elements)
                tensor = torch.cat(result_tensors)
        else:
